...
export default function PainelHome() {
  ...
  const carregarGrafico = async (dias: number) => {
    ...
    const formatado = Object.keys(agrupado)
      .sort()
      .map((k) => ({ data: k.split("-").reverse().join("/"), valor: agrupado[k] }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        <div className="bg-black text-white p-6 rounded-lg w-full text-center">
          <div className="text-sm">Notas Negociadas</div>
          <div className="text-3xl font-bold mt-2">{notas}</div>
        </div>
        <div className="bg-black text-white p-6 rounded-lg w-full text-center">
          <div className="text-sm">Volume Negociado</div>
      ...
      <div className="bg-white shadow rounded-lg p-4 mt-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold">Volume Negociado (últimos {dias} dias)</h3>
...
PainelHome.getLayout = (page: React.ReactNode) => <PainelLayout>{page}</PainelLayout>;
